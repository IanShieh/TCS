...
2. 若為原始 `DingxinErpTemplate` 模板（URL 含 `DingxinErpTemplate`）：
   - `speckit.specify` / `speckit.plan` / `speckit.tasks` 僅產生文件，在模板內執行**也可以**（不會污染程式碼）
   - 但 `speckit.implement` **必須**在從模板 Clone 出來的新目錄中執行
   - 建議提醒使用者先 clone：`git clone <template-url> C:\projects\{作業代號}`
3. 若已在新作業目錄中 → 直接繼續，無需額外動作
